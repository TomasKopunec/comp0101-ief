...
5. Try run a example impl file, provided by the official document, create a yaml file and name it `example.yml`, copy the following content into it:
   ...
   ```bash
   yarn impact-engine --impl example.yml --output example-output.yml
   ```
...
6. Try run a couple more example impl files, which located in the `examples/impl` directory, for example:
   ```bash
   yarn impact-engine --impl examples/impl/test/metadata.yml --output examples/ompl/metadat-output.yml
   ```
